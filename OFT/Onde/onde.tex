...
\Delta x  =N \lambda \qquad \Delta t = NT 
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\begin{scope}[shift={(0,-1.35*\R)}]
				\draw[blue,thick,samples=100,smooth,variable=\x]
	...
	\subsubsection{Osservatore in movimento}

	% MOVING - DOPPLER SHIFT
	\begin{tikzpicture}
		\def\s{0.61}
		\draw[blue!20,dashed,very thin,dash pattern=on 1 off 1]
		({-(\N-0.25)*\s*\lam},0) -- (0,0);
		\fill[red!20] ({-(\N-0.25)*\s*\lam},0) circle (0.06);
		\waves{\s}
	...
	\begin{tikzpicture}
		...
		(-\s*\R,-0.27*\R) --++ (0,0.5*\R)
		(0,-0.27*\R) --++ (0,0.5*\R);
		\draw[blue!50,dashed,very thin,dash pattern=on 1 off 1]
		({-\s*\R},0) -- (0,0);
		\fill[red!20] ({-\s*\R},0) circle (0.06);
		\foreach \i [evaluate={\r=\i*\lam;\a=\ang-1.2*\i;}] in {1,...,\N}{
			\draw[blue,thick] ({-\i*\s*\lam},0)++(180-\a:\r) arc(180-\a:180+\a:\r);
			\draw[blue,thick] ({-\i*\s*\lam},0)++(    \a:\r) arc(    \a:-\a:\r);
		}
