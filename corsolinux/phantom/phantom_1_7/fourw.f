...
************************************************************************
      subroutine fourw(p1,p2,p3,p4,p5,p6,p7,p8,
...
*zw                                                                     
      if (i_ww.gt.0) then
          ...
          if (p1(0).gt.0.d0) then
            if (id1.eq.idw(1).or.(i_ww.eq.2.and.id1.eq.idw(3)))
     &           iresw(1)=1
          endif
          iresw(3)=0
          if (p3(0).gt.0.d0) then
            if (id3.eq.idw(1).or.(i_ww.eq.2.and.id3.eq.idw(3)))
     &           iresw(3)=1
          endif
          iresw(5)=0
...
*zw                                                                     
      cauxmw2=cmw2
      if (i_ww.eq.2.and.i_osp.eq.2.and.iresw(1).eq.1) then
...
*zw                                                                     
      iceps1=0
      iceps2=0
      if (iresw(1).eq.1) then
*  compute the W momentum                                               
        do mu=0,3
          pw(mu)=p1(mu)+p2(mu)
        enddo
        if (id1.eq.idw(1)) then
          jpol=ipolw(1)
        elseif (id1.eq.idw(3)) then
          jpol=ipolw(2)
        else
...
* pol w transverse                                                      
        ...
        if (iceps1.eq.1) then
* p.q -- p.q=cscal,p=ceps1,q=cw12%e,bef=,aft=
      cscal=(ceps1(0)*cw12%e(0)-ceps1(1)*cw12%e(1)-ceps1(2)*cw12
     & %e(2)-ceps1(3)*cw12%e(3))
  
          do mu=0,3
            cw12%e(mu)=cscal*conjg(ceps1(mu))
          enddo
        endif
  
        if (iceps2.eq.1) then
* p.q -- p.q=cscal1,p=ceps1,q=cw12%e,bef=,aft=
      cscal1=(ceps1(0)*cw12%e(0)-ceps1(1)*cw12%e(1)-ceps1(2)*cw1
     & 2%e(2)-ceps1(3)*cw12%e(3))
  
* p.q -- p.q=cscal2,p=ceps2,q=cw12%e,bef=,aft=
      cscal2=(ceps2(0)*cw12%e(0)-ceps2(1)*cw12%e(1)-ceps2(2)*cw1
     & 2%e(2)-ceps2(3)*cw12%e(3))
  
          do mu=0,3
            cw12%e(mu)=cscal1*conjg(ceps1(mu))
...
*zw                                                                     
      cauxmw2=cmw2
      if (i_ww.eq.2.and.i_osp.eq.2.and.iresw(3).eq.1) then
        cauxmw2=rmw2*cuno-cim*gamw*rmw
      endif
      cdw=-wcl/(s34-cauxmw2)
*      cdw=-wcl/(s34-cmw2)                                              
...
*zw                                                                     
      iceps1=0
      iceps2=0
      if (iresw(3).eq.1) then
*  compute the W momentum                                               
        do mu=0,3
          pw(mu)=p3(mu)+p4(mu)
        enddo
        if (id3.eq.idw(1)) then
          jpol=ipolw(1)
        elseif (id3.eq.idw(3)) then
          jpol=ipolw(2)
        else
          print *, 'ERROR, MISMATCH IN POLARIZATIONS'
          stop
...
* pol w transverse                                                      
        ...
        if (iceps1.eq.1) then
* p.q -- p.q=cscal,p=ceps1,q=cw34%e,bef=,aft=
      cscal=(ceps1(0)*cw34%e(0)-ceps1(1)*cw34%e(1)-ceps1(2)*cw34
     & %e(2)-ceps1(3)*cw34%e(3))
  
          do mu=0,3
            cw34%e(mu)=cscal*conjg(ceps1(mu))
          enddo
        endif
  
        if (iceps2.eq.1) then
* p.q -- p.q=cscal1,p=ceps1,q=cw34%e,bef=,aft=
      cscal1=(ceps1(0)*cw34%e(0)-ceps1(1)*cw34%e(1)-ceps1(2)*cw3
     & 4%e(2)-ceps1(3)*cw34%e(3))
  
* p.q -- p.q=cscal2,p=ceps2,q=cw34%e,bef=,aft=
      cscal2=(ceps2(0)*cw34%e(0)-ceps2(1)*cw34%e(1)-ceps2(2)*cw3
     & 4%e(2)-ceps2(3)*cw34%e(3))
  
          do mu=0,3
            cw34%e(mu)=cscal1*conjg(ceps1(mu))
     &           +cscal2*conjg(ceps2(mu))
          enddo
        endif
      endif
...
*zw                                                                     
      if(iresw(1).eq.1) then
        do mu=0,3
          lz1_234(mu)%a(2)=czero
          lz1_234(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p1,q=p234
...
*zw                                                                     
      if(iresw(1).eq.1) then
        do mu=0,3
          lz1_278(mu)%a(2)=czero
          lz1_278(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p1,q=p278
...
*zw                                                                     
      if(iresw(3).eq.1) then
        do mu=0,3
          lz3_412(mu)%a(2)=czero
          lz3_412(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p3,q=p412
...
*zw                                                                     
      if(iresw(3).eq.1) then
        do mu=0,3
          lz3_456(mu)%a(2)=czero
          lz3_456(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p3,q=p456
...
*zw                                                                     
      if(iresw(5).eq.1) then
        do mu=0,3
          lz5_634(mu)%a(2)=czero
          lz5_634(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p5,q=p634
...
*zw                                                                     
      if(iresw(7).eq.1) then
        do mu=0,3
          lz7_812(mu)%a(2)=czero
          lz7_812(mu)%c(2)=czero
        enddo
      else
* quqd -- p=p7,q=p812
...
* _312(mu)%b,prq=p312q,bef=cz1234(mu)+,aft=
     ...
     & 3_12%a(2)*rz4_312(mu)%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1234(mu),a1=l3_12%a,c1=l3_12%c,a2=rf4_312(mu)%a,b2=rf4
* _312(mu)%b,prq=p312q,bef=cf1234(mu)+,aft=
      cf1234(mu)=cf1234(mu)+(l3_12%c(2)*p312q*rf4_312(mu)%b(1)+l
     & 3_12%a(2)*rf4_312(mu)%a(2))
      end do
      do mu=0,3
...
* _534(mu)%b,prq=p534q,bef=cz5634(mu)+,aft=
     ...
     & 5_34%a(2)*rz6_534(mu)%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5634(mu),a1=l5_34%a,c1=l5_34%c,a2=rf6_534(mu)%a,b2=rf6
* _534(mu)%b,prq=p534q,bef=cf5634(mu)+,aft=
      cf5634(mu)=cf5634(mu)+(l5_34%c(2)*p534q*rf6_534(mu)%b(1)+l
     & 5_34%a(2)*rf6_534(mu)%a(2))
      end do
      do mu=0,3
...
* _712(mu)%b,prq=p712q,bef=cz1278(mu)+,aft=
     ...
     & 7_12%a(2)*rz8_712(mu)%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1278(mu),a1=l7_12%a,c1=l7_12%c,a2=rf8_712(mu)%a,b2=rf8
* _712(mu)%b,prq=p712q,bef=cf1278(mu)+,aft=
      cf1278(mu)=cf1278(mu)+(l7_12%c(2)*p712q*rf8_712(mu)%b(1)+l
     & 7_12%a(2)*rf8_712(mu)%a(2))
      end do
      do mu=0,3
...
* _756(mu)%b,prq=p756q,bef=cz5678(mu)+,aft=
     ...
     & 7_56%a(2)*rz8_756(mu)%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5678(mu),a1=l7_56%a,c1=l7_56%c,a2=rf8_756(mu)%a,b2=rf8
* _756(mu)%b,prq=p756q,bef=cf5678(mu)+,aft=
      cf5678(mu)=cf5678(mu)+(l7_56%c(2)*p756q*rf8_756(mu)%b(1)+l
     & 7_56%a(2)*rf8_756(mu)%a(2))
      end do
  
*    Then those of the type                                             
*                                                                       
*       (mu) _Z___|                      (mu) _gamma__|                 
*                 |_W__/       and                    |_W__/            
*                 |    \                              |    \            
*                                                                       
...
* b2=r2_34%b,prq=p234q,bef=cz1234(mu)+,aft=
     ...
     & z1_234(mu)%a(2)*r2_34%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1234(mu),a1=lf1_234(mu)%a,c1=lf1_234(mu)%c,a2=r2_34%a,
* b2=r2_34%b,prq=p234q,bef=cf1234(mu)+,aft=
      cf1234(mu)=cf1234(mu)+(lf1_234(mu)%c(2)*p234q*r2_34%b(1)+l
     & f1_234(mu)%a(2)*r2_34%a(2))
      end do
      do mu=0,3
...
* b2=r2_78%b,prq=p278q,bef=cz1278(mu)+,aft=
     ...
     & z1_278(mu)%a(2)*r2_78%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1278(mu),a1=lf1_278(mu)%a,c1=lf1_278(mu)%c,a2=r2_78%a,
* b2=r2_78%b,prq=p278q,bef=cf1278(mu)+,aft=
      cf1278(mu)=cf1278(mu)+(lf1_278(mu)%c(2)*p278q*r2_78%b(1)+l
     & f1_278(mu)%a(2)*r2_78%a(2))
      end do
      do mu=0,3
...
* b2=r4_12%b,prq=p412q,bef=cz1234(mu)+,aft=
     ...
     & z3_412(mu)%a(2)*r4_12%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1234(mu),a1=lf3_412(mu)%a,c1=lf3_412(mu)%c,a2=r4_12%a,
* b2=r4_12%b,prq=p412q,bef=cf1234(mu)+,aft=
      cf1234(mu)=cf1234(mu)+(lf3_412(mu)%c(2)*p412q*r4_12%b(1)+l
     & f3_412(mu)%a(2)*r4_12%a(2))
      end do
      do mu=0,3
...
* b2=r4_56%b,prq=p456q,bef=cz5634(mu)+,aft=
     ...
     & z3_456(mu)%a(2)*r4_56%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5634(mu),a1=lf3_456(mu)%a,c1=lf3_456(mu)%c,a2=r4_56%a,
* b2=r4_56%b,prq=p456q,bef=cf5634(mu)+,aft=
      cf5634(mu)=cf5634(mu)+(lf3_456(mu)%c(2)*p456q*r4_56%b(1)+l
     & f3_456(mu)%a(2)*r4_56%a(2))
      end do
      do mu=0,3
...
* b2=r6_34%b,prq=p634q,bef=cz5634(mu)+,aft=
     ...
     & z5_634(mu)%a(2)*r6_34%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5634(mu),a1=lf5_634(mu)%a,c1=lf5_634(mu)%c,a2=r6_34%a,
* b2=r6_34%b,prq=p634q,bef=cf5634(mu)+,aft=
      cf5634(mu)=cf5634(mu)+(lf5_634(mu)%c(2)*p634q*r6_34%b(1)+l
     & f5_634(mu)%a(2)*r6_34%a(2))
      end do
      do mu=0,3
...
* b2=r6_78%b,prq=p678q,bef=cz5678(mu)+,aft=
     ...
     & z5_678(mu)%a(2)*r6_78%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5678(mu),a1=lf5_678(mu)%a,c1=lf5_678(mu)%c,a2=r6_78%a,
* b2=r6_78%b,prq=p678q,bef=cf5678(mu)+,aft=
      cf5678(mu)=cf5678(mu)+(lf5_678(mu)%c(2)*p678q*r6_78%b(1)+l
     & f5_678(mu)%a(2)*r6_78%a(2))
      end do
      do mu=0,3
...
* b2=r8_12%b,prq=p812q,bef=cz1278(mu)+,aft=
     ...
     & z7_812(mu)%a(2)*r8_12%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf1278(mu),a1=lf7_812(mu)%a,c1=lf7_812(mu)%c,a2=r8_12%a,
* b2=r8_12%b,prq=p812q,bef=cf1278(mu)+,aft=
      cf1278(mu)=cf1278(mu)+(lf7_812(mu)%c(2)*p812q*r8_12%b(1)+l
     & f7_812(mu)%a(2)*r8_12%a(2))
      end do
      do mu=0,3
...
* b2=r8_56%b,prq=p856q,bef=cz5678(mu)+,aft=
     ...
     & z7_856(mu)%a(2)*r8_56%a(2))
      ...
      do mu=0,3
* TLTR0_W -- aa=cf5678(mu),a1=lf7_856(mu)%a,c1=lf7_856(mu)%c,a2=r8_56%a,
* b2=r8_56%b,prq=p856q,bef=cf5678(mu)+,aft=
      cf5678(mu)=cf5678(mu)+(lf7_856(mu)%c(2)*p856q*r8_56%b(1)+l
     & f7_856(mu)%a(2)*r8_56%a(2))
      end do
...
*     &            (ch1278*ch5634)/(p1278q-cmh2)                        
        if (i_signal.eq.0)then
          cres2w_h_2w=(ch1234*ch5678)*
     &         (rhww**2/(p1234q-cmh2)+rhhww**2/(p1234q-cmhh2))+
     &            (ch1278*ch5634)*
     &         (rhww**2/(p1278q-cmh2)+rhhww**2/(p1278q-cmhh2))
...
*        paux(m)=p12(m)+p56(m)                                          
*       enddo                                                           
*p.q      !subroutine p.q[p.q,p,q,bef,aft]                              
*skin paux paux 0 0                                                     
*       print*, 'pre-skin=', skin                                       
  
       call unit_4w(p12,s12,cw12,p34,s34,cw34,
     &    p56,s56,cw56,p78,s78,cw78,cunit)
  
* NEEd to check more carefully the                                      
*  factor due to conventions in the code                                
       cres4wquad=cres4wquad+cunit
      endif
*end diogo                                                              
...
* sig                                                                   
*      cres=cresUD_3w+cres2w_f_2w+cres2w_z_2w+cres2w_h_2w+cres4wquad    
      if (i_signal.eq.0) then
        cres=cresUD_3w+cres2w_f_2w+cres2w_z_2w+cres2w_h_2w+cres4wquad
* six                                                                   
*      elseif (i_signal.eq.1) then                                      
...
* for every external particle                                           
      ...
      cres=cres/sqrt(p1k0*p2k0*p3k0*p4k0*p5k0*p6k0*p7k0*p8k0)
