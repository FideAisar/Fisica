...
* giovanni 2017/9/6 begin
* integrating complex mass and nogamwz programs
*zw
*      common/polww/i_ww,i_pol,i_polwp,i_polwm,i_polwel,i_polwmu            
*      common/osp/i_osp
*zwend
* giovanni 2017/9/6 end

c      DATA rmw/80.385d0/, rmz/91.1876d0/, rmt/173.21d0/, rmb/4.75d0/,
c     &     rmc/0.75d0/,rmtau/1.78d0/,
c     &     gamw/0.2085d+01/, gamz/0.24952d+01/
      DATA rmw/80.3579736098775d0/, rmz/91.1534806191827d0/, 
     &     rmt/173.21d0/, rmb/4.75d0/, rmc/0.75d0/,rmtau/1.78d0/,
     &     gamw/0.2084298998278219d+01/, gamz/0.2494266378772824d+01/
      DATA gf/1.16637d-05/, alfainv_me/137.0359895d0/,
...
c      print *, 'alpha_ew^-1 as PHANTOM does= ', 1.d0/alfainv
c      print *, 'alpha_ew =', (sqrt(2.d0)/pi)*gf*rmw2*(1.d0-rmw2/rmz2)
      ...
      rcotw=rcw/sw
      rcot2w=rcotw**2
c      print*, 's2w,g2,elcharge2,alfainv,sw,rc2w,rcw,rcotw,rcot2w'
c      print*, s2w,g2,elcharge2,alfainv,sw,rc2w,rcw,rcotw,rcot2w
...
* integrating complex mass and nogamwz programs  
*zw
*      if (i_ww.eq.1.and.i_osp.gt.0) then
      if ((i_ww.eq.2.or.i_zz.eq.2.or.(i_ww.eq.1.and.i_zz.eq.1))
     &     .and.i_osp.eq.2) then
*zwend
       cmw2=rmw2*cuno
       cmz2=rmz2*cuno
...
c      print*, 's2w,g2,elcharge2,alfainv,sw,rc2w,rcw,rcotw,rcot2w'
c      print*, s2w,g2,elcharge2,alfainv,sw,rc2w,rcw,rcotw,rcot2w     
c      print*, 'cmw,cmz,cmw2,cmz2,sw,rcw,rcotw,rcot2w'
c      print*, cmw,cmz,cmw2,cmz2,sw,rcw,rcotw,rcot2w
c      print *, 'alpha_ew^-1 as PHANTOM does= ', 1.d0/alfainv      
c      print*, '+++++++++++++++++++++++++++++++++++++++++++++++++'      
*cmassend
...
* heavh
      ...
      if (rmh.ge.0.d0.and.i_singlet.ne.1) then
        rhbb=-rmb_run/(2.d0*cmw*sw) *ghfactor
        rhtt=-rmt_run/(2.d0*cmw*sw) *ghfactor
        rhww=cmw/sw *ghfactor
        rhzz=cmz/sw/rcw *ghfactor
        r3h=-3.d0*rmh2/(sw*2.d0*cmw) *ghfactor
...
c the others cim**11, hence we give a - sign to it with respect to the others.
        ...
        r4h=+3.d0*rmh2/(4.d0*s2w*cmw2) *ghfactor
        if (i_hh.eq.1) then
          rhhbb=-rmb_runhh/(2.d0*cmw*sw) *ghhfactor
          rhhtt=-rmt_runhh/(2.d0*cmw*sw) *ghhfactor
          rhhww=cmw/sw *ghhfactor
          rhhzz=cmz/sw/rcw *ghhfactor
c prova
c          r3hh=-3.d0*rmh2/(sw*2.d0*cmw) *ghhfactor
          r3hh=-3.d0*rmhh2/(sw*2.d0*cmw) *ghhfactor
c provaend
          r2hh2w=-1.d0/(2.d0*s2w)*ghhfactor
          r2hh2z=-1.d0/(2.d0*s2w*rc2w) *ghhfactor
c prova
c          r4hh=+3.d0*rmh2/(4.d0*s2w*cmw2) *ghhfactor
          r4hh=+3.d0*rmhh2/(4.d0*s2w*cmw2) *ghhfactor
c provaend
          r1h1hh2w=0.d0
          r1h1hh2z=0.d0
          r1h2hh=0.d0
...
***********  i_singlet
      if (rmh.ge.0.d0.and.i_singlet.eq.1) then
        ...
        rhbb=-rmb_run/(2.d0*cmw*sw) *rcosa
        rhtt=-rmt_run/(2.d0*cmw*sw) *rcosa
        rhww=cmw/sw *rcosa
        rhzz=cmz/sw/rcw *rcosa

        r3h=-3.d0*rmh2/(sw*2.d0*cmw)*
...
c the others cim**11, hence we give a - sign to it with respect to the others.
        ...
        r4h=+3.d0/(4.d0*s2w*cmw2) *(rmh2*
     &       (rcosa6-2.d0*rcosa3*rsina3*tgbeta+rsina6*tgbeta2)
     &       +rmhh2*(rcosa4*rsina2+2.d0*rcosa3*rsina3*tgbeta+
     &       rcosa2*rsina4*tgbeta2))      

        rhhbb=-rmb_runhh/(2.d0*cmw*sw) *rsina
        rhhtt=-rmt_runhh/(2.d0*cmw*sw) *rsina
        rhhww=cmw/sw *rsina
        rhhzz=cmz/sw/rcw *rsina

        r3hh=-3.d0*rmhh2/(sw*2.d0*cmw)*
     &       (rsina3+rcosa3*tgbeta)
        r2hh2w=-1.d0/(2.d0*s2w)*rsina2
        r2hh2z=-1.d0/(2.d0*s2w*rc2w) *rsina2
c        r4hh=+3.d0*rmh2/(4.d0*s2w*cmw2) *rsina
        r4hh=+3.d0/(4.d0*s2w*cmw2) *(rmh2*
     ...
     &       rcosa6*tgbeta2))      
        ...
        r1h2hh=rmhh2/(cmw*sw)*(-rcosa3*rsina2-rcosa*rsina4+
     &       rcosa4*rsina*tgbeta+rcosa2*rsina3*tgbeta)
     &      +rmh2/(2.d0*cmw*sw)*(-rcosa3*rsina2-rcosa*rsina4+
     &       rcosa4*rsina*tgbeta+rcosa2*rsina3*tgbeta)
        r2h1hh=-rmh2/(cmw*sw)*(rcosa4*rsina+rcosa*2*rsina3+
     &       rcosa3*rsina2*tgbeta+rcosa*rsina4*tgbeta)
     &      -rmhh2/(2.d0*cmw*sw)*(rcosa4*rsina+rcosa2*rsina3+
     &       rcosa3*rsina2*tgbeta+rcosa*rsina4*tgbeta) 

        r3h1hh=+3.d0/(4.d0*s2w*cmw2) *(rmh2*
     ...
     &       rcosa3*rsina3*tgbeta2))
        ...
        r2h2hh=+3.d0/(4.d0*s2w*cmw2) *(rmh2*
     ...
     &       +rcosa*rsina5*tgbeta+rcosa4*rsina2*tgbeta2))
        ...
        r1h3hh=+3.d0/(4.d0*s2w*cmw2) *(rmh2*
     &       (rcosa3*rsina3-rcosa4*rsina2*tgbeta+rcosa2*rsina4*tgbeta
...
* Zeta-electron left and right
      czer=(+1.d0*s2w)/rcw/sw
      czel=(-.5d0+1.d0*s2w)/rcw/sw
...
* Zeta-quarkup left and right
      fqul=2.d0/3.d0
      fqur=fqul
      czqur=(-2.d0/3.d0*s2w)/rcw/sw
      czqul=(.5d0-2.d0/3.d0*s2w)/rcw/sw
* Zeta-quarkdown left and right
      fqdl=-1.d0/3.d0
      fqdr=fqdl
      czqdr=(1.d0/3.d0*s2w)/rcw/sw
      czqdl=(-.5d0+1.d0/3.d0*s2w)/rcw/sw
* W left (right=0)
      wcl=1.d0/sw/sqrt(2.d0)

c      print*,'czqdr,czqdl,wcl'
c      print*,czqdr,czqdl,wcl
