/*
 *analisys_LHC_4jmunu_Rainwater.C
 *
 *Last update: Aug 31, 2007
 *
 * Introduced additional cuts to conform to Raiwater analysis
 * NB: b's are kept separated from ordinary jets for pT and eta_max cuts
 ******************************************************************************/
...
void analysis_LHC_4jmunu(TString opt, Double_t rmh) {
  ...
  TLorentzVector p_bj[8];        //momentum of bj system
  Int_t index_jet_bj[3];
  bool is_a_b[8];
  ...
  Double_t DR,DeltaEta_VV;
  
  //user-defined selection cuts
  TString setcut_E_min_j, setcut_E_min_clep, setcut_pT_min_j, 
    setcut_pT_min_clep, setcut_eta_max_j, setcut_eta_max_clep, 
    setcut_Mjj, setcut_Mjcjc, setcut_deltaetajfjb, setcut_Wlv, 
    setBveto, setBsingletag, setBdoubletag, setTopcut_jjj, setTopcut_jlv_rec,
    setcut_DRHj,setcut_deltaetaVV,setcut_TotVisM;
  Double_t cut_E_min_j, cut_E_min_clep, cut_pT_min_j, cut_pT_min_clep, 
    ...
    cut_DRHj,cut_deltaetaVV,cut_TotVisM;

  // Rainwater cuts
  TString setcut_sign_etajfXetajb,setcut_DRjj,setcut_DRjl,
    setcut_centrality_bl,setcut_pT_min_b,setcut_eta_max_b;
  Double_t cut_DRjj,cut_DRjl,cut_centrality_bl,cut_pT_min_b,cut_eta_max_b;

  //random number generator and the corresponding random variable
  TRandom3 irand;
   
  static Int_t ipasscuts,ipassDR,ipass_centrality;
  static Int_t ibtag;
 ...
 ******************************************************************************/
  ...
  //Minimum pT for jets
  cin>>setcut_pT_min_j>>dummy;
  if(setcut_pT_min_j=="y"){
    cin>>cut_pT_min_j>>dummy;
  }

  //Minimum pT for bs
  cin>>setcut_pT_min_b>>dummy;
  if(setcut_pT_min_b=="y"){
    cin>>cut_pT_min_b>>dummy;
  }

  //Minimum pT for charged leptons
  cin>>setcut_pT_min_clep>>dummy;
  if(setcut_pT_min_clep=="y"){
    cin>>cut_pT_min_clep>>dummy;
  }

  //Maximum pseudorapidity for jets
  cin>>setcut_eta_max_j>>dummy;
  if(setcut_eta_max_j=="y"){
    cin>>cut_eta_max_j>>dummy;
  }

  //Maximum pseudorapidity for b's
  cin>>setcut_eta_max_b>>dummy;
  if(setcut_eta_max_b=="y"){
    cin>>cut_eta_max_b>>dummy;
  }

  //Maximum pseudorapidity for charged  leptons
  cin>>setcut_eta_max_clep>>dummy;
  if(setcut_eta_max_clep=="y"){
    cin>>cut_eta_max_clep>>dummy;
  }

  //Mimimum invariant mass of any two jets
  cin>>setcut_Mjj>>dummy;
  if(setcut_Mjj=="y"){
    cin>>cut_Mjj>>dummy;
  }

  //Cut on invariant mass of the two central jets
  cin>>setcut_Mjcjc>>dummy;
  if(setcut_Mjcjc=="y"){
  ...
  cin>>setcut_deltaetajfjb>>dummy;
  if(setcut_deltaetajfjb=="y"){
    cin>>cut_deltaetajfjb >>dummy;
  }

  //Cut on the sign of the product eta(jf)*eta(jb)
  cin>>setcut_deltaetajfjb>>dummy;

  //Cut on minimum DeltaR of any two jets 
  cin>>setcut_DRjj>>dummy;
  if(setcut_DRjj=="y"){
    cin>>cut_DRjj>>dummy; 
  }

  //Cut on minimum DeltaR of any jet and any charged lepton 
  cin>>setcut_DRjl>>dummy;
  if(setcut_DRjl=="y"){
    cin>>cut_DRjl>>dummy; 
  }
...
//  cut_Mjlv_rec = 15.0; // => window = centralMjlv +/- cutMjlv
  ...
  cin>>Mcut>>dummy;     //M_min_VVsytem
    ...
    //Check compatibility of b-cut setup
  ...
  cin>>setBsingletag>>dummy;      // accepts single b-tagged events only
  cin>>setBdoubletag>>dummy;      // accepts double b-tagged events only
 ...
 ******************************************************************************/
  ...
  text_cuts.AddText(" ");
  if(setcut_pT_min_j=="y"){
    TString s="minimum p_{T} of light jets:";
    s+=cut_pT_min_j;
    s+=" GeV";  
    text_cuts.AddText(s.Data());
  }
  if(setcut_pT_min_b=="y"){
    TString s="minimum p_{T} of b's:";
    s+=cut_pT_min_b;
    s+=" GeV";  
    text_cuts.AddText(s.Data());
  }
  ...
  if(setcut_eta_max_j=="y"){
    TString s="maximum |#eta| of light jets:";
    s+=cut_eta_max_j;
    text_cuts.AddText(s.Data());
  }
  if(setcut_eta_max_b=="y"){
    TString s="maximum |#eta| of b's:";
    s+=cut_eta_max_b;
    text_cuts.AddText(s.Data());
  }
  if(setcut_eta_max_clep=="y"){
  ...
  if(setcut_sign_etajfXetajb=="y"){
    TString s="#eta_{jf}*#eta_{jb} <0";
    text_cuts.AddText(s.Data());
  }
  if(setcut_DRjj=="y"){
    TString s="minimum DeltaR between any two jets:";
    s+=cut_DRjj;
    text_cuts.AddText(s.Data());
  }
  if(setcut_DRjl=="y"){
    TString s="minimum DeltaR between any jet and any charged lepton:";
    s+=cut_DRjl;
    text_cuts.AddText(s.Data());
  }
  if(setcut_centrality_bl=="y"){
    TString s="minimum eta separation between any (l,b) and if or jb:";
    s+=cut_centrality_bl;
    text_cuts.AddText(s.Data());
  }
  if(setTopcut_jjj=="y"){
  ...
  if(setBsingletag=="y" || setBdoubletag=="y"){
    TString s="c-quark fake efficiency:";
    s+=Cfake_efficiency;
  ...
  title="Difference in pseudorapidity between tag jets at the Higgs peak with selection cuts";
 ...
 // At the Higgs peak
  ...
  TH1D hew_MinM_Vhadj_peak_jcjc("hew_MinM_Vhadj_peak_jcjc","M_{jV} (GeV)",nbins_M,Mjjlv_low,Mjjlv_high);
  title="Minimum inv. mass of tag jet + had vector boson at the jcjc peak"+TString(app);
  hew_MinM_Vhadj_peak_jcjc.SetTitle(title.Data()); 

  TH1D hew_pT_jcjc_peak("hew_pT_jcjc_peak","p_{T} (GeV)",nbins_pT,pT_low,pT_high);
  title="pT of the two central jets at the peak"+TString(app);
  hew_pT_jcjc_peak.SetTitle(title.Data());

  //Delta eta| between reconstructed heavy bosons
  TH1D hew_DeltaEta_VV_peak_jcjc("hew_DeltaEta_VV_peak_jcjc","|#eta|",nbins_eta,0,2*eta_high);
  title="|#Delta #eta| between reconstructed heavy bosons at the jcjc peak"+TString(app);
  hew_DeltaEta_VV_peak_jcjc.SetTitle(title.Data());

  TH1D hew_Mvis_peak_jcjc("hew_Mvis_peak_jcjc",";M_{jj} (GeV)",nbins_M,Mjj_low,Mjj_high);
  hew_Mvis_peak_jcjc.SetTitle("Total visible Invariant mass at the jcjc peak");
...
//  hew_DR_bj_peak.SetTitle(title.Data());
  ...
  TH1D *hew_Mvis_peak_jcjc_buf = (TH1D*) hew_Mvis_peak_jcjc.Clone();
  TH1D *hew_MinM_Vhadj_peak_jcjc_buf = (TH1D*) hew_MinM_Vhadj_peak_jcjc.Clone();
  TH1D *hew_pT_jcjc_peak_buf = (TH1D*) hew_pT_jcjc_peak.Clone();
  TH1D *hew_DeltaEta_VV_peak_jcjc_buf = (TH1D*) hew_DeltaEta_VV_peak_jcjc.Clone();
  ...
  for(Int_t k_n=0;k_n<nentries_tree;k_n++) {
   ...
   // Increase jet count
    ...
    for(Int_t k=0;k<n_j;k++){
      if(setcut_pT_min_j=="y"){
	if(!is_a_b[k] && jet[k].Pt() < cut_pT_min_j){
	  ipasscuts=0;
	}
	if(is_a_b[k] && jet[k].Pt() < cut_pT_min_b){
	  ipasscuts=0;
	}
      }
      if(setcut_eta_max_j=="y"){
	if(!is_a_b[k] && TMath::Abs(jet[k].Eta()) > cut_eta_max_j){
	  ipasscuts=0;
	}
	if(is_a_b[k] && TMath::Abs(jet[k].Eta()) > cut_eta_max_b){
	  ipasscuts=0;
	}
      }
    }
...
// Rainwater extra cuts
  ...
  // DeltaR among jets and charged leptons
      if(setcut_DRjl=="y"){
	for(Int_t ki=0;ki<n_j;ki++){
	  for(Int_t kj=0;kj<n_clep;kj++){
	    DR = DeltaR(jet[ki],clep[kj]);
	    if(DR < cut_DRjl) {
	      ipasscuts=0;
	    }
	  }
  ...
  // Centrality of charged leptons
      if(setcut_centrality_bl=="y"){
	for(Int_t kj=0;kj<n_clep;kj++){
	  if( jf.Eta()-cut_centrality_bl < clep[kj].Eta() ||
              jb.Eta()+cut_centrality_bl > clep[kj].Eta() ) {
	    ipasscuts=0;
	  }
	}
      }
...
//Fill Mbb histogram before cuts
    ...
    if(TMath::Abs(V[0].M()-rmh)<5.0){
      ...
      if(ibtag==2 && setBdoubletag=="y"){
        DR = DeltaR(particle[index_b[0]],particle[index_b[1]]);
...
//**********************************************************************
      ...
      if(ibtag==2 && setBdoubletag=="y"){
 // Minimum DeltaR between H=bb system and one other jet
        ipassDR=1;
        if(setcut_DRHj=="y"){
          DR=1.e4;
          for(Int_t k=0;k<n_j;k++) {
            if(index_jet[k]!= index_b[0]  && index_jet[k]!=index_b[1]){ 
              auxdelta = DeltaR(p_bb,particle[index_jet[k]]);
              if(auxdelta < DR){
             DR = auxdelta;
              }
            }
        ...
        }   // End if(setcut_DRHj=="y")
        
 WRONG
  // Centrality of b's
      ipass_centrality=1;
      if(setcut_centrality_bl=="y"){
	for(Int_t kj=0;kj<ibtag;kj++){
	  if( jf.Eta()-cut_centrality_bl < particle[index_jet[kj]].Eta() ||
              jb.Eta()+cut_centrality_bl > particle[index_jet[kj]].Eta() ) {
	    ipass_centrality=0;
	  }
	}
      }
        if(ipassDR==1 && ipass_centrality==1) {
...
//Order masses
 ...
 // Minimum DeltaR between H=bj1 system and one other jet
        ipassDR=1;
        if(setcut_DRHj=="y"){
          DR=1.e4;
          for(Int_t k=0;k<n_j;k++) {
            if(index_jet[k]!= index_jet_bj[index_sort[0]]
                && index_jet[k]!=is_a_b){ 
              auxdelta = DeltaR(p_bj[index_sort[0]],particle[index_jet[k]]);
              if(auxdelta < DR){
             DR = auxdelta;
              }
            }
...
//**********************************************************************
 ...
 // Minimum DeltaR between H=jcjc system and one other jet
        ipassDR=1;
        if(setcut_DRHj=="y"){
          auxa=V[0].Eta()-jf.Eta();
          auxb=V[0].DeltaPhi(jf);
          DR = TMath::Sqrt(auxa*auxa+auxb*auxb);
          auxdelta = DeltaR(V[0],jb);
          if(auxdelta < DR){
             DR = auxdelta;
              }
          if(DR < cut_DRHj){
        ...
        if(ipassDR==1) {
          ...
          Vhad_jb = V[0]+jb;
          if(TMath::Abs(V[0].M()-rmh)<5.0){
          hew_MinM_Vhadj_peak_jcjc_buf->Fill(TMath::Min(Vhad_jf.M(),Vhad_jb.M()));
          hew_pT_jcjc_peak_buf->Fill(V[0].Pt());
          hew_DeltaEta_VV_peak_jcjc_buf->Fill(DeltaEta_VV);
          hew_Mvis_peak_jcjc_buf->Fill(tot_vis.M());
          }
        }
...
//	else if(MinDEta_Vtag == DEta_jblv){
  ...
  if(opt=="xs") {
    ...
    hew_MinM_Vhadj_peak_jcjc_buf->Scale(scale_Mjjlv);
    hew_Mvis_peak_jcjc_buf->Scale(scale_Mjjlv);
    hew_pT_jcjc_peak_buf->Scale(scale_pT);
    ...
    hew_DeltaEta_VV_peak_jcjc_buf->Scale(xsec/(((2*eta_high)/nbins_eta)*(nentries_tree)));
  ...
  hew_MinM_Vhadj_peak_jcjc.Add(hew_MinM_Vhadj_peak_jcjc_buf);
  hew_pT_jcjc_peak.Add(hew_pT_jcjc_peak_buf);
  hew_DeltaEta_VV_peak_jcjc.Add(hew_DeltaEta_VV_peak_jcjc_buf);
  hew_Mvis_peak_jcjc.Add(hew_Mvis_peak_jcjc_buf);
  ...
  hew_MinM_Vhadj_peak_jcjc_buf->Reset();
  hew_Mvis_peak_jcjc_buf->Reset();
  hew_DeltaEta_VV_peak_jcjc_buf->Reset();
  hew_pT_jcjc_peak_buf->Reset();
...
//  TString s;
  ...
  hew_MinM_Vhadj_peak_jcjc.Write();
  hew_DeltaEta_VV_peak_jcjc.Write();
  hew_pT_jcjc_peak.Write();
  hew_Mvis_peak_jcjc.Write();
  fo->cd();
