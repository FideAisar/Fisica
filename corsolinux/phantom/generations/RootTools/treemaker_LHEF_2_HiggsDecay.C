 ...
 *
 * This particular version decays all Higgs particle (id 25) isotropically
 * to b-bbar, multiplying the cross section by the appropriate BR
 ******************************************************************************/
...
void treemaker()
{
  ...
  Int_t imotherstatus;       //Les Houches mother status:
                             ...
                             // state
  Int_t ipart_id;             // temporary particle id 
  ...
  TRandom3 irand;
  TLorentzVector pH,paux;
  TVector3 b;
  Double_t rmb,brH2bb,ctheta,stheta,ran_phi,Eaux,pmodaux;
  ...
  cout<<endl;
  // Multiply by BR(H-->bb)
  cout<<"Enter the value of BR(H-->bb): ";
  cin>>brH2bb;
  cout.precision(10);
  cout<<endl;
  cout<<"read total cross section: xs = "<<xsec<<" pb"<<endl;
  cout<<"read  BR(H-->bb)             = "<<brH2bb<<endl;
  xsec*=brH2bb;
  cout<<"(total cross section)*BR : xs = "<<xsec<<" pb"<<endl;
  cout<<endl;
  TString sxsec;
  sxsec.Form("%f",xsec);
  ...
  cout<<"Enter the value of b mass (GeV): ";
  cin>>rmb;
  cout.precision(10);
  cout<<"b mass : rmb = "<<rmb<<" Gev"<<endl;
  cout<<endl;
  cout<<endl;
  ...
  // Scan LHEF file____________________________________________________________
	...
	nline_local++;
	sscanf(&line_aux[0], "%i %d %*i %*i %*i %*i %le %le %le %le %le %le %le",&ipart_id,&imotherstatus,&dummy_double[0],&dummy_double[1],&dummy_double[2],&dummy_double[3],&dummy_double[4],&dummy_double[5],&dummy_double[6]);

	if(TMath::Abs(imotherstatus)==1){
// Check if Higgs
          if(ipart_id==25){
             pH = TLorentzVector(dummy_double[0],dummy_double[1],dummy_double[2],dummy_double[3]);
             b = pH.BoostVector();
     // Random b momentum
             ctheta=irand.Uniform( -1.e0, 1.e0);
             stheta=TMath::Sqrt(1.e0-ctheta*ctheta);
             ran_phi=irand.Uniform( 0.e0, 2.e0*TMath::Pi());
             Eaux=dummy_double[4]/2.e0;
             pmodaux=TMath::Sqrt(Eaux*Eaux-rmb*rmb);
             paux=TLorentzVector(pmodaux*stheta*cos(ran_phi),pmodaux*stheta*sin(ran_phi),pmodaux*ctheta,Eaux);
//    Test
//    cout.precision(10);
//    cout<<paux[0]<<"  "<<paux[1]<<"  "<<paux[2]<<"  "<<paux[3]<<" b momentum"<<endl;
//    cout<<paux.M()<<" b mass"<<endl;
   
//     Boost  
             paux.Boost(b);
             px[nparticle]=paux.Px();
             py[nparticle]=paux.Py();
             pz[nparticle]=paux.Pz();
             E[nparticle]=paux.E();
             idup[nparticle]=5;
             istup[nparticle]=1;
             nparticle++;
//    Test
//    cout.precision(10);
//    cout<<paux[0]<<"  "<<paux[1]<<"  "<<paux[2]<<"  "<<paux[3]<<" b momentum after boost"<<endl;
//    cout.precision(10);
//    cout<<paux.M()<<" b mass after boost"<<endl;
   
     // b bar 
             px[nparticle]=pH.Px()-paux.Px();
             py[nparticle]=pH.Py()-paux.Py();
             pz[nparticle]=pH.Pz()-paux.Pz();
             E[nparticle]=pH.E()-paux.E();
             idup[nparticle]=-5;
             istup[nparticle]=1;
             nparticle++;
             }
