 ...
 *
 *This particular version eliminates all events with final state b's
 *in order to combine W4j with the enriched sample with final state b's 
 ******************************************************************************/
...
void treemaker()
{
  ...
  Int_t imotherstatus;       //Les Houches mother status:
                             ...
                             // state
  Double_t xsec,xsec_nob;             //total cross section
  ...
  // Scan LHEF file____________________________________________________________
	...
	if(TMath::Abs(imotherstatus)==1){
	  ...
	  nparticle++;
          if(TMath::Abs(idup[nparticle])==5 && istup[nparticle]==1){
              n_FinalStateB++;
              }
	}
      }
   ...
   cout<<endl<<"Total cross section with final b's "<<xsec<<" pb"<<endl;
// Rescale cross section
  xsec_nob=xsec*nevts_accepted/nevts;
   cout<<endl<<"Total cross section without final b's "<<xsec_nob<<" pb"<<endl;
  TString sxsec;
  sxsec.Form("%f",xsec_nob);
