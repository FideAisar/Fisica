...
void treemaker(TString filename = "phamom.dat")
{
  ...
  FILE *fp0 = fopen(filename.Data(),"r");
  ...
  // open the ASCII event file
  FILE *fp = fopen(filename.Data(),"r");

  /*
  //create a ROOT file with the same extension (e.g. _qcd or _pew) of input file
  TString rootfile = filename;
  rootfile.Remove(rootfile.Length()-3,rootfile.Length());
  rootfile.Remove(0,6);
  rootfile.Prepend("treedata");
  rootfile.Append("root");
  */

  TString rootfile = "tree.root";


  TFile *f = new TFile(rootfile.Data(),"RECREATE");
  cout<<"created ROOT file "<<rootfile.Data()<<endl;
  ...
  // Scan input file________________________________________________________
  ...
  cout<<endl;
  cout<<"number of lines read in "<<filename.Data()<<" = "
      <<nline+ninitlines<<endl;
