...
\]
Questo è un aspetto molto importante della forza magnetica su una carica in moivmento, la qale permetto in ogni caso di definire un acmpo elettrico di origine magnetica che chiamiamo \textit{campo elettromotore} \(\vec{E}_{em}\).

Il campo elettromotore provoca una separazione delle cariche positive dalle cariche negative, fatto che genere un secondo campo elettrostatico \(\vec{E}_H\) detto \textit{campo di Hall} che si oppone a un ulteriore accumulo.
...
Prendiamo in esame un corpo di un certo materiale investito dal campo magnetico di un solenoide. Si nota sperimentalmente in base al materiale di cui è costituito il corpo questo viene respinto e attratto.
Chiamiamo \textit{diamagnetici} i materiali che vengono debolmente respinti e \textit{paramagnetici} quelli che vengono debolmente attratti.

Il materiale, investito dal campo magnetico \(\vec{B}\), acquisisce un momento di dipolo \(\vec{\mu}\) proporzionale a \(\vec{B}\):
\[
\begin{array}{rc}
	\text{Diamagnetici: } & \vec{\mu} = - |k| \vec{B} \\
	\text{Paramagnetici: } & \vec{\mu} = + |k| \vec{B}
\end{array}
\]
...
\end{equation}
dove \(n\) è il numero di atomi/molecole per unità di volume.
